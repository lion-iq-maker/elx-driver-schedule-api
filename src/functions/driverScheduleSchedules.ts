...
async function createDriverSchedule(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const payload = isSqlConfigured() ? await resolveTypedDriverAndTruck(body ?? {}, actorEmail) : body ?? {};
...
async function resolveTypedDriverAndTruck(payload: unknown, actorEmail: string): Promise<unknown> {
