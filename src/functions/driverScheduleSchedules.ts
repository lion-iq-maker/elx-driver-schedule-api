...
async function createDriverSchedule(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (isSqlConfigured()) {
    ...
    const conflict = shouldCheckConflict(validation.value.status) ? await findSchedulePayloadConflictInSql(validation.value) : null;
  ...
  const conflict = shouldCheckConflict(schedule.status) ? findScheduleConflict(schedule) : null;
  if (conflict) return jsonResponse({ errors: [conflict] }, 409);
...
function shouldCheckConflict(status: ScheduleStatus): boolean {
  return ["Scheduled", "Picked Up", "In Transit", "At Depot", "Delayed"].includes(status);
}
...
async function updateDriverSchedule(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (isSqlConfigured()) {
    ...
    const conflict = shouldCheckConflict(validation.value.status) ? await findSchedulePayloadConflictInSql(validation.value, id) : null;
    if (conflict) return jsonResponse({ errors: [conflict] }, 409);
  ...
  const conflict = shouldCheckConflict(updated.status) ? findScheduleConflict(updated, id) : null;
  if (conflict) return jsonResponse({ errors: [conflict] }, 409);
...
async function resolveTypedAssets(payload: unknown, actorEmail: string): Promise<unknown> {
  ...
  const driverName = getTypedValue(draft, "driverName", "driver", "displayName");
  const truckNumber = getTypedValue(draft, "truckNumber", "truck", "truckNumber");
  const trailerNumber = getTypedValue(draft, "trailerNumber", "trailer", "trailerNumber");

  if (driverName && !hasValue(draft.driverId)) {
    const driver = await findOrCreateDriverInSql(driverName, actorEmail);
    draft.driverId = driver.id;
  }

  if (truckNumber && !hasValue(draft.truckId)) {
    const truck = await findOrCreateTruckInSql(truckNumber, actorEmail);
    draft.truckId = truck.id;
  }

  if (trailerNumber && !hasValue(draft.trailerId)) {
...
function getTypedValue(draft: Record<string, unknown>, directKey: string, objectKey: string, objectValueKey: string): string {
  const directValue = draft[directKey];
  if (typeof directValue === "string" && directValue.trim()) return directValue.trim();

  const plainValue = draft[objectKey];
  if (typeof plainValue === "string" && plainValue.trim()) return plainValue.trim();

  if (plainValue && typeof plainValue === "object") {
    const nestedValue = (plainValue as Record<string, unknown>)[objectValueKey];
