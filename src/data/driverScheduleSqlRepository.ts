...
export async function findOrCreateDriverInSql(displayName: string, actorEmail: string): Promise<DriverLookup> {
  const normalizedName = displayName.trim();
  const pool = await getSqlPool();
  const existing = await pool.request()
    .input("displayName", sql.NVarChar(150), normalizedName)
    .query<{ id: string; display_name: string; email: string | null; status: string }>(`
      SELECT TOP 1 id, display_name, email, status
      FROM dbo.drivers
      WHERE is_deleted = 0 AND display_name = @displayName;
    `);

  const row = existing.recordset[0];
  if (row) return { id: row.id, displayName: row.display_name, email: row.email, status: row.status };
  ...
  await pool.request()
    ...
    .input("actorEmail", sql.NVarChar(255), actorEmail)
    .query(`
      INSERT INTO dbo.drivers (id, display_name, status, created_by, updated_by)
      VALUES (@id, @displayName, N'Available', @actorEmail, @actorEmail);
    `);

  return { id, displayName: normalizedName, email: null, status: "Available" };
}

export async function findOrCreateTruckInSql(truckNumber: string, actorEmail: string): Promise<TruckLookup> {
  const normalizedNumber = truckNumber.trim();
  const pool = await getSqlPool();
  const existing = await pool.request()
    .input("truckNumber", sql.NVarChar(50), normalizedNumber)
    .query<{ id: string; truck_number: string; status: string; current_location: string | null }>(`
      SELECT TOP 1 id, truck_number, status, current_location
      FROM dbo.trucks
      WHERE is_deleted = 0 AND truck_number = @truckNumber;
    `);

  const row = existing.recordset[0];
  if (row) return { id: row.id, truckNumber: row.truck_number, status: row.status, currentLocation: row.current_location };
  ...
  await pool.request()
    ...
    .input("actorEmail", sql.NVarChar(255), actorEmail)
    .query(`
      INSERT INTO dbo.trucks (id, truck_number, status, created_by, updated_by)
      VALUES (@id, @truckNumber, N'Available', @actorEmail, @actorEmail);
    `);

  return { id, truckNumber: normalizedNumber, status: "Available", currentLocation: null };
}

export async function findOrCreateTrailerInSql(trailerNumber: string, actorEmail: string): Promise<TrailerLookup> {
  const normalizedNumber = trailerNumber.trim();
  const pool = await getSqlPool();
  const existing = await pool.request()
    .input("trailerNumber", sql.NVarChar(50), normalizedNumber)
    .query<{ id: string; trailer_number: string; trailer_type: string | null; status: string }>(`
      SELECT TOP 1 id, trailer_number, trailer_type, status
      FROM dbo.trailers
      WHERE is_deleted = 0 AND trailer_number = @trailerNumber;
    `);

  const row = existing.recordset[0];
  if (row) return { id: row.id, trailerNumber: row.trailer_number, trailerType: row.trailer_type, status: row.status };
  ...
  await pool.request()
    ...
    .input("actorEmail", sql.NVarChar(255), actorEmail)
    .query(`
      INSERT INTO dbo.trailers (id, trailer_number, trailer_type, status, notes)
      VALUES (@id, @trailerNumber, N'General', N'Available', CONCAT(N'Created from schedule by ', @actorEmail));
    `);

  return { id, trailerNumber: normalizedNumber, trailerType: "General", status: "Available" };
}
